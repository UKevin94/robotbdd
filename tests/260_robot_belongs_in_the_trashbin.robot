*** Settings ***
Documentation    robot_belongs_in_the_trashbin
Metadata         ID                           260
Metadata         Reference                    rf_1
...
Test Setup
    ...
    ...                You can define the ${TEST_260_SETUP} variable with a keyword for setting up this specific test.
    ...                If both are defined, ${TEST_260_SETUP} will be run after ${TEST_SETUP}.

    ${TEST_SETUP_VALUE} =        Get Variable Value    ${TEST_SETUP}
    ${TEST_260_SETUP_VALUE} =    Get Variable Value    ${TEST_260_SETUP}
    IF    $TEST_SETUP_VALUE is not None
        Run Keyword    ${TEST_SETUP}
    END
    IF    $TEST_260_SETUP_VALUE is not None
        Run Keyword    ${TEST_260_SETUP}
    END

Test Teardown
    [Documentation]    test teardown
    ...                You can define the ${TEST_TEARDOWN} variable with a keyword for tearing down all your tests.
    ...                You can define the ${TEST_260_TEARDOWN} variable with a keyword for tearing down this specific test.
    ...                If both are defined, ${TEST_TEARDOWN} will be run after ${TEST_260_TEARDOWN}.

    ${TEST_260_TEARDOWN_VALUE} =    Get Variable Value    ${TEST_260_TEARDOWN}
    ${TEST_TEARDOWN_VALUE} =        Get Variable Value    ${TEST_TEARDOWN}
    IF    $TEST_260_TEARDOWN_VALUE is not None
        Run Keyword    ${TEST_260_TEARDOWN}
    END
